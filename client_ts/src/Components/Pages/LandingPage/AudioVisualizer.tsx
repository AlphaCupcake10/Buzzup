            ...
            <div className="p-4 w-full h-full rounded-md flex items-center justify-between">
                {
                    (()=>{
                        ...
                        {
                            arr.push(<div key={i} style={{filter:`hue-rotate(${i/count*360*hueRotateFactor}deg)`,animationDelay:`${i*timePeriodFactor + Math.random()*randomFactor}ms`}} className={`${styles.bar} w-1 sm:w-2 h-0 bg-accent rounded-full`}></div>)
                        }
                        return arr;
                    })()
                }
